    ...
    public partial class WIthdrawMoneyForm : Form
    {
        ...
        private async void Dgv_CellDoubleClick ( object sender, DataGridViewCellEventArgs e )
        {
            if ( e.RowIndex >= 0 ) // Ensure a valid row index
            {
                ...
                using ( WithdrawMoneyDialog modalForm = new WithdrawMoneyDialog (accountHolders.FirstOrDefault (holder => holder.Email == targetEmail)) )
                {
        ...
        private async void WIthdrawMoneyForm_Load ( object sender, EventArgs e )
        {
